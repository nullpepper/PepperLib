...
class BukkitMemoryWorldService implements InstanceWorldService {
    ...
    public CompletableFuture<WorldInstance> create(final WorldInstanceRequest request) {
        ...
        final String worldName = WORLD_NAME_PREFIX + UUID.randomUUID().toString().replace("-", "");
        final Entry entry = new Entry(new BukkitWorldInstance(instanceId, worldName, request.template().id()));
    ...
    private WorldInstance loadOnMain(final Entry entry, final String worldName) {
        if (Bukkit.getWorld(worldName) != null) {
            throw new java.util.concurrent.CompletionException(
                    new WorldProviderException(WorldProviderError.WORLD_LOAD_FAILED, "world already exists: " + worldName));
        }
        final World world = Bukkit.createWorld(new WorldCreator(worldName));
        if (world == null) {
            throw new java.util.concurrent.CompletionException(
                    new WorldProviderException(WorldProviderError.WORLD_LOAD_FAILED, "Bukkit.createWorld returned null: " + worldName));
        }
